...
{{ ... }}
    ...
    def overlay_subtitles_and_logo(
        ...
        # Ensure ASS Fontname uses the family name (not a .ttf path, which
        # libass cannot resolve) and that the font directory is available.
        from src.render.steps import normalize_ass_font_path, escape_ffmpeg_filter_path
        normalize_ass_font_path(ass_path)

        # Escape special characters in ASS path for FFmpeg filter
        # FFmpeg requires : \ ' , and [ ] to be escaped in filter paths
        ass_path_escaped = escape_ffmpeg_filter_path(ass_path)

        # Point libass at the directory holding the configured font file so
        # the family name resolves identically on macOS and the worker image.
        fonts_dir = str(Path(self.font_path).resolve().parent)
        subs_spec = f"fontsdir={escape_ffmpeg_filter_path(fonts_dir)}"
        ...
        subs_filter = f"{subtitles_filter_name}={ass_path_escaped}:{subs_spec}"
            ...
            if crop_params:
                ...
                if overlay_prepped:
                    filter_complex = (
                        ...
                        f"[subbed]{subs_filter}[subbed_with_subs];"
        ...
        # Choose encoding settings based on mode and requested encoder
        from src.render.steps import resolve_video_encoder
        audio_bitrate = "192k" if fast_mode else "320k"
        video_codec_args, fps_args, encoder_name = resolve_video_encoder(encoder, fast=fast_mode)
        if fast_mode:
            print(f"⚡ Using FAST mode ({encoder_name} if available, else ultrafast)")
    ...
    def generate_karaoke(self, video_path: str, srt_content: Optional[str] = None, output_path: str = None, logo_path: Optional[str] = None, fast_mode: bool = False, ass_file: Optional[str] = None, effect_overlay_paths: Optional[List[str]] = None, max_duration_seconds: Optional[float] = None, overlay_hue_deg: Optional[float] = None, overlay_saturation: Optional[float] = None, overlay_tint_color: Optional[str] = None, encoder: str = "auto") -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            if effect_overlay_paths:
                ...
                # Enforce prepped overlays only: use the configured overlays directory
                overlay_dir = settings.karaoke_overlay_dir
                if not os.path.isabs(overlay_dir):
                    overlay_dir = str(Path(__file__).resolve().parents[1] / overlay_dir)
